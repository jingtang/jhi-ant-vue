...
@Component
export default class CommonQueryComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public prepareRemove(instance: ICommonQuery): void {
  ...
  public removeById(removeId: number): void {
    this.commonQueryService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.commonQueryCommonQuery.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: ICommonQuery): void {
    if (this.showInOther || this.editInModal) {
      this.commonQueryId = row.id;
