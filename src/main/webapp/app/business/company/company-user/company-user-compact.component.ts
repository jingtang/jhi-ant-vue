...
@Component
export default class CompanyUserCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public companyUserCommonTableData: ICommonTable = null; // 实体模型数据
  ...
  public prepareRemove(instance: ICompanyUser): void {
  ...
  public removeById(removeId: number): void {
    this.companyUserService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.companyCompanyUser.deleted', { param: this.removeId }).toString();
  ...
  newCompanyUser() {
    ...
    this.editStatus[companyUser.id].edit = true;
  }
