...
@Component
export default class CompanyCustomerCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public companyCustomerCommonTableData: ICommonTable = null; // 实体模型数据
  public mapOfFilter: { [key: string]: any } = {
    children: { list: [], value: [], type: 'one-to-many' },
    companyUsers: { list: [], value: [], type: 'one-to-many' },
    companyBusinesses: { list: [], value: [], type: 'one-to-many' },
  ...
  public prepareRemove(instance: ICompanyCustomer): void {
  ...
  public removeById(removeId: number): void {
    this.companyCustomerService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.companyCompanyCustomer.deleted', { param: this.removeId }).toString();
  ...
  newCompanyCustomer() {
    ...
    companyCustomer.id = -1;
    this.companyCustomers.push(companyCustomer);
    this.editStatus[companyCustomer.id].edit = true;
  }
  ...
  public initRelationships(): void {
    ...
    forkJoin([this.companyCustomerService().tree()]).subscribe(
      ([companyCustomers]) => {
        ...
        const listOfFilterparent = companyCustomers.data.slice(0, companyCustomers.data.length > 8 ? 7 : companyCustomers.data.length - 1);
