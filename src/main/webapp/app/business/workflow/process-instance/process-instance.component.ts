...
import { InstanceData } from '@/shared/model/workflow/instance-data.model';
import { IProcessDefinition } from '@/shared/model/workflow/process-definition.model';

@Component
export default class ProcessDefinitionComponent extends mixins(JhiDataUtils, Vue2Filters.mixin, AlertMixin) {
  ...
  public startInstance(row: IProcessDefinition): void {
    const instanceData = new InstanceData();
    instanceData.processDefinitionId = row.id;
    instanceData.returnVariables = true;
    instanceData.businessKey = 'myleave';
    this.processInstanceService()
      .start(instanceData)
      .subscribe(
        res => {
  ...
  getCommonTableData() {
    this.xGridColumns = [
      ...
      { field: 'processDefinitionUrl', title: '流程定义URL' },
