...
@Component
export default class ProcessTableConfigCompactComponent extends mixins(JhiDataUtils, Vue2Filters.mixin, AlertMixin) {
  ...
  public processTableConfigCommonTableData: ICommonTable = null; // 实体模型数据
  ...
  public removeById(removeId: number): void {
    this.processTableConfigService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.workflowProcessTableConfig.deleted', { param: this.removeId }).toString();
