...
@Component
export default class ProcessEntityRelationCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public processEntityRelations: IProcessEntityRelation[] = [];
  public processEntityRelationCommonTableData: ICommonTable = null; // 实体模型数据
  ...
  public prepareRemove(instance: IProcessEntityRelation): void {
  ...
  public removeById(removeId: number): void {
    this.processEntityRelationService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.workflowProcessEntityRelation.deleted', { param: this.removeId }).toString();
  ...
  // 新增Article 并设置为可编辑状态
  newProcessEntityRelation() {
    const processEntityRelation = new ProcessEntityRelation();
    processEntityRelation.id = -1;
    this.processEntityRelations.push(processEntityRelation);
    this.editStatus[processEntityRelation.id].edit = true;
  }
