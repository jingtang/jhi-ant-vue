...
@Component
export default class GpsInfoCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public removeById(removeId: number): void {
    this.gpsInfoService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.settingsGpsInfo.deleted', { param: this.removeId }).toString();
  ...
  newGpsInfo() {
    ...
    this.editStatus[gpsInfo.id].edit = true;
  }
