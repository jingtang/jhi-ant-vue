...
@Component
export default class AdministrativeDivisionCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public searchValue = '';
  administrativedivisionsNzTreeNodes: any[]; // todo 有时可能多余。
  ...
  public removeById(removeId: number): void {
    this.administrativeDivisionService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.settingsAdministrativeDivision.deleted', { param: this.removeId }).toString();
  ...
  // 新增Article 并设置为可编辑状态
  newAdministrativeDivision() {
    const administrativeDivision = new AdministrativeDivision();
    administrativeDivision.id = -1;
    this.administrativeDivisions.push(administrativeDivision);
    this.editStatus[administrativeDivision.id].edit = true;
  }
