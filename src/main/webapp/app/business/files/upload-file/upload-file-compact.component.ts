...
@Component
export default class UploadFileCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  public removeById(removeId: number): void {
    this.uploadFileService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.filesUploadFile.deleted', { param: this.removeId }).toString();
