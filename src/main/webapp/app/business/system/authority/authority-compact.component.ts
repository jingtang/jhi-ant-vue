...
@Component
export default class AuthorityCompactComponent extends mixins(Vue2Filters.mixin, AlertMixin) {
  ...
  authoritiesCollectionNzTreeNodes: any[]; // todo 有时可能多余。
  ...
  public removeById(removeId: number): void {
    this.authorityService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhiAntVueApp.systemAuthority.deleted', { param: this.removeId }).toString();
