...
export default class LeaveComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('leave-heading'));
...
export class LeaveDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('jhiAntVueApp.workflowLeave.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-leave'));
}
